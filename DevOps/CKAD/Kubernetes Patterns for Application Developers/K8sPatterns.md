...
* Provide an identity to Pods in the cluster, like there are identity systems for users
* Stored in and managed by the cluster
* Scoped to Namespace. Names of SAs should be unique within Namespaces
* By default every namespace has a default service account and it does not have any permissions beyond that of an authenticated user

#### Access Control
* SAs are compatible with Role-Based Access Control (RBAC)
* RBAC large to define roles and associated permissions to access K8s APIs and resources.
* A K8s administrator usually defines the roles and allocates permissions
...
* For a Pod, in the `describe` output, `serviceAccount` field needs to be set for a non-default SA.
* The token is mounted in `/var/run/secrets/kubernetes.io` path

#### Image Pull Secrets
* They authenticate with private container registries
    * It could be a docker registry server address, username and password
* SAs can also store image pull secrets. They can provide both an authentication token and image pull secrets
* It creates a neat separation of responsibilities when we use SAs to manage image pull secrets rather than hard-coding it Pod specs
...
#### Objective
* Increase efficiency and get the most out of kubectl
...
* Copy command for your shell.
* Execute and add to bash_profile to ensure it is run at the start of every shell creation

#### `get` Tricks
* List the short names for resources using `kubectl api-resources`
* Filter out `get` command output by using labels.
    * To list the labels use the `—show-label` argument along with your `get` command
    * Then filter by `-L` followed by comma-separated label names
    * Filter for only those with labels using `-l`
    * Filter for specific values of labels using `=` and `!=`. e.g. `-l <label-name>=<label-value>`
* Can sort using `—sort-by`
    * For e.g., `kubectl get pods -n kube-system —sort-by=metadata.creationTimestamp`
    * Better way to specify json path: `kubectl get pods -n kube-system —sort-by=‘(.metadata.creationTimestamp)'`
* Use -o wide for more fields
* Use `jsonpath` output format alternatively to the `yaml` format
    * `kubectl get pods -n kube-system —sort-by=‘(.status.podIP)’ -o jsonpath=‘(.items[*].status.podIP)’`

#### Creating new resources
* Use `—dry-run` to stop kubectl from creating the resource. Instead use it along with `-o` to generate the manifest and store it in a file
* Create from file / directory using `create -f` command
* Works for delete as well. Will look for resources referenced in the file and deletes them
* One other way is to use the output of `get` command to create other resources
    * `kubectl get pod -n kube-system cube-proxy-hwjjt -o yaml —export`. `—export` removes unnecessary output lines

#### Explain
* `kubectl explain pod` - describes top level fields in a Pod
* `kubectl explain pod.spec.containers.resources | more` - to traverse and describe
* `kubectl explain pod.spec.containers —recursive` - to explain all nested fields
